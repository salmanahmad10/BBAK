...
class Compiler:
    def __init__(self, style):
        ...
        self.root = Node.Node("body", None, self.content_holder)
    ...
    def compile(self, generated_gui):
        ...
        for token in dsl_file:
            ...
            if token.find(self.opening_tag) != -1:
                ...
                element = Node.Node(token, current_parent, self.content_holder)
                ...
                tokens = token.split(",")
                for t in tokens:
                    element = Node.Node(t, current_parent, self.content_holder)
                    current_parent.add_child(element)
