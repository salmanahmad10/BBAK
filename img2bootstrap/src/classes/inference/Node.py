...
class Node:
    ...
    def rendering_function(self, key, value):
        if key.find("btn") != -1:
            value = value.replace(TEXT_PLACE_HOLDER, SamplerUtils.SamplerUtils.get_random_text())
        elif key.find("title") != -1:
            value = value.replace(TEXT_PLACE_HOLDER, SamplerUtils.SamplerUtils.get_random_text(length_text=5, space_number=0))
        elif key.find("text") != -1:
            value = value.replace(TEXT_PLACE_HOLDER,
                                  SamplerUtils.SamplerUtils.get_random_text(length_text=56, space_number=7, with_upper_case=False))
        return value
